/**
 * @file fbcons.cpp
 * @brief AArch64 framebuffer console — renders text via GOP framebuffer.
 *
 * The rendering logic (PSF font, scrolling, cursor blink) is identical
 * to the x86 version.  late_init() reads the GOP framebuffer address
 * from boot_info (set by the UEFI bootloader) and maps it via mmio_map.
 */
...
// Colors (32-bit BGRX)
static constexpr uint32_t FG_COLOR = 0x00AAAAAA;
static constexpr uint32_t BG_COLOR = 0x00000000;
...
static constexpr uint32_t CURSOR_BLINK_RATE = 50;
static constexpr uint32_t CURSOR_COLOR = 0x00AAAAAA;
