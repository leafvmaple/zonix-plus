...
- [x] **README.md 路径过时** *(已修复)*: `./tools/` → `./scripts/`，调试目标统一为 `make debug`。
...
- [x] **内嵌 TODO 注释 (5 处)** *(已审计)*: 位置已验证，内容为未来功能规划，保留不动：
  - `stdio.cpp:206` — cprintf 返回值
  - `ahci.cpp:34` — AHCI 硬编码设备大小
  - `kernel/fs/fat/fat_core.cpp` — FAT write 支持
