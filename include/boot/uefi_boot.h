...
static void* memcpy(void* dst, const void* src, uintptr_t n) {
...
static void* memset(void* dst, int c, uintptr_t n) {
    auto* d = static_cast<uint8_t*>(dst);
    while (n--) {
...
static EFI_STATUS uefi_load_kernel_file(EFI_BOOT_SERVICES* bs, EFI_HANDLE image_handle, void** buf, uintptr_t* size) {
    ...
    for (int i = 0; paths[i]; i++) {
        status = root->Open(root, &file, (wchar_t*)paths[i], EFI_FILE_MODE_READ, 0);
        if (!EFI_ERROR(status)) {
            break;
        }
    }
    ...
    *size = (uintptr_t)file_info->FileSize;
    bs->FreePool(file_info);
