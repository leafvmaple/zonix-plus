...
# Write MBR
dd if=${BINDIR}/mbr.bin of="$IMAGE" bs=446 count=1 conv=notrunc 2>/dev/null
...
MTOOLSRC=/tmp/mtoolsrc_zonix mcopy -i "$IMAGE"@@512 ${BINDIR}/kernel ::KERNEL.SYS
...
dd if=${BINDIR}/bootloader.bin of="$IMAGE" bs=1 seek=1024 conv=notrunc 2>/dev/null
