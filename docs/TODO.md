...
- ✨ **C++17 内核**：全局 new/delete、RAII `intr::Guard`、C++ 构造函数链
...
- [ ] **实现信号量**
  - 文件：`kernel/sync/sem.cpp`
  - 实现：P、V 操作
  - 应用：生产者-消费者

- [ ] **实现互斥锁**
  - 文件：`kernel/sync/mutex.cpp`
  - 实现：lock、unlock
  - 优化：避免自旋

- [ ] **实现条件变量**
  - 文件：`kernel/sync/cond.cpp`
...
- **当前完成度**：约 45%（完整基础设施 + 双引导 + 64 位 + 进程调度 + 磁盘/文件系统）
